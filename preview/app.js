...
if (key === "growth" && cur && cur.pop) {
    ...
    if (key === "future" && cur) {
      ...
      function getUVal(i){ return i===0 ? cur.u : cur["u_r"+i]; }
      var uTrendPhrase = withTrendMeaning("u", trendSincePhrase(getUVal, 5, 6));
      if (KK && KK[curName] && KK[curName].ka3 != null) {
        ...
        if (!ka3Near) {
          ...
          if (!uHighF && !ka3High) { ka3Judge = "中央値より低め"; ka3Body = "<strong style='color:#3a9970;'>[財政]</strong>将来への借金・<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化、どちらの面から見ても軽い状態です。"; }
            ...
            ka3Judge = "中央値より高め";
            var ka1TrendA = kkMetricTrend(KK[curName], "ka1", KK[curName].ka1);
          ...
          function getKa3Val(i){ return i===0 ? KK[curName].ka3 : KK[curName]["ka3_r"+i]; }
          var ka3TrendPhrase = withTrendMeaning("ka3", trendSincePhrase(getKa3Val, 4, 5));
          topSummaryHtml += kkCrossBox("🏛️ 総務省公式の組み合わせ分析",
            "将来負担比率", cur.u+"%", uHighF?"重め":"軽め",
            "施設の老朽化率", ka3v+"%", ka3Judge,
            curName + "は公会計でみた有形固定資産減価償却率（老朽化率）が" + ka3v + "%です。" + ka3Body,
            false, uTrendPhrase, ka3TrendPhrase);
      ...
      if (KK && KK[curName] && KK[curName].ka1 != null) {
        ...
        if (!ka1Near4) {
          ...
          else { ka1Judge4 = "中央値より低め"; ka1Body = "<strong style='color:#c0623a;'>[公会計]</strong>資産規模・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも小さめです。資産の規模と将来世代への負担のバランスは取れている状態です。"; }
          function getKa1Val(i){ return i===0 ? KK[curName].ka1 : KK[curName]["ka1_r"+i]; }
          var ka1TrendPhrase = withTrendMeaning("ka1", trendSincePhrase(getKa1Val, 4, 5));
          topSummaryHtml += kkCrossBox("🏛️ 総務省公式の組み合わせ分析",
            "将来負担比率", cur.u+"%", uHighF?"重め":"軽め",
            "住民一人当たり資産額", ka1v4+"万円", ka1Judge4,
            curName + "は公会計でみた住民一人当たり資産額が" + ka1v4 + "万円です。" + ka1Body,
            false, uTrendPhrase, ka1TrendPhrase);
          kkCrossBoxShownF = true;
        }
      }
      if (KK && KK[curName] && KK[curName].ka6 != null) {
        ...
        if (!ka6Near4) {
          ...
          if (!uHighF && !ka6High4) { ka6Judge4 = "中央値より低め"; ka6Body = "<strong style='color:#c0623a;'>[公会計]</strong>行政コスト・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも軽めです。コンパクトな運営と言えます。"; }
          ...
          function getKa6Val(i){ return i===0 ? KK[curName].ka6 : KK[curName]["ka6_r"+i]; }
          var ka6TrendPhrase = withTrendMeaning("ka6", trendSincePhrase(getKa6Val, 4, 5));
          topSummaryHtml += kkCrossBox("🏛️ 総務省公式の組み合わせ分析",
            "将来負担比率", cur.u+"%", uHighF?"重め":"軽め",
            "住民一人当たり行政コスト", ka6v4+"万円", ka6Judge4,
            curName + "は公会計でみた住民一人当たり行政コストが" + ka6v4 + "万円です。" + ka6Body,
            false, uTrendPhrase, ka6TrendPhrase);
          kkCrossBoxShownF = true;
        }
      }
...
/* ===== 公会計機能 ===== */
  ...
  function trendSincePhrase(getVal, maxN, currentYear) {
    var vals = [];
  ...
  function kkPerCapitaTrendNote(entry, code, curVal, g, unit, label) {
    if (!entry) return "";
    var oldVal = entry[code + "_r4"];
    var td = trendDescribe(oldVal, curVal, unit || "万円", 1);
    if (!td) return "";
    var subj = label ? label + "は" : "";
    if (td.dir === "増加") {
      return subj + "この5年で" + td.text + "しています。";
    }
    if (td.dir === "減少") {
      return subj + "この5年で" + td.text + "しています。";
    }
    ...
    if (popGrowing) {
      return subj + "この5年で" + td.text + "おり、人口が増えている一方でこの金額はあまり伸びていません。人口増加に整備・サービス拡充が追いついていない可能性も考えられます。";
    }
    return subj + "この5年で" + td.text + "おり、老朽化による目減り分と新規投資がほぼ相殺されている可能性も考えられます。";
  }
  var KK_CROSSCHECK_CAVEAT = "<div style='background:rgba(160,139,232,0.08);border-radius:10px;padding:12px 14px;margin-top:10px;'>" +
    ...
    "</div>";
  function kkCrossBox(heading, zaiLabel, zaiVal, zaiJudge, kkLabel, kkVal, kkJudge, bodyText, reverse, zaiTrendPhrase, kkTrendPhrase) {
  ...
  function kkReciprocalCross(code, entryV, isPref, entry) {
    ...
    if (code === "ka4" && cur.sfs && cur.sfs > 0) {
      ...
      function getRRatio(i){
        var rv = i===0 ? cur.r : cur["r_r"+i];
        var sv = i===0 ? cur.sfs : cur["sfs_r"+i];
        return (rv!=null && sv) ? rv/sv*100 : null;
      }
      var rTrendPhrase = withTrendMeaning("r", trendSincePhrase(getRRatio, 5, 6));
      function getKa4Val(i){ return i===0 ? entry.ka4 : entry["ka4_r"+i]; }
      var ka4TrendPhrase = entry ? withTrendMeaning("ka4", trendSincePhrase(getKa4Val, 4, 5)) : null;
      return kkCrossBox("🔗 財政と合わせて見ると", "すぐ使える貯金", ratioRec.toFixed(1)+"%", judgeRec, "自己資本の厚み", entryV+"%", ka4HighRec?"中央値より高め":"中央値より低め", curName + "の財政調整基金は標準財政規模の" + ratioRec.toFixed(1) + "%です。" + bodyRec, true, rTrendPhrase, ka4TrendPhrase) + KK_CROSSCHECK_CAVEAT;
    }
    if (code === "ka7" && cur.d != null) {
      ...
      if (!dHighRec && !ka7HighRec) { judgeD = "軽め"; bodyD = "借金の重さは、<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担・<strong style='color:#c0623a;'>[公会計]</strong>残高の規模どちらから見ても軽めです。"; }
        ...
        bodyD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は軽めに見えますが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体は大きめです。" + (ka7TrendC === "declining" ? "着実に返済が進んでいる長期返済中、という可能性も考えられます。" : "新しい借入も続いており、返済はこれから本格化する可能性も考えられます。");
      }
      else {
        judgeD = "重め";
        var ka7TrendD = kkMetricTrend(entry, "ka7", entryV);
        bodyD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は重めですが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体はそれほど大きくありません。" + (ka7TrendD === "declining" ? "短期集中で返済を終えつつある可能性も考えられます。" : "返済期間を短く設定している可能性も考えられます。");
      }
      function getDVal(i){ return i===0 ? cur.d : cur["d_r"+i]; }
      var dTrendPhrase = withTrendMeaning("d", trendSincePhrase(getDVal, 5, 6));
      function getKa7Val(i){ return i===0 ? entry.ka7 : entry["ka7_r"+i]; }
      var ka7TrendPhrase = entry ? withTrendMeaning("ka7", trendSincePhrase(getKa7Val, 4, 5)) : null;
      return kkCrossBox("🔗 財政と合わせて見ると", "毎年の返済負担", cur.d+"%", judgeD, "借金残高そのもの", entryV+"万円", ka7HighRec?"中央値より高め":"中央値より低め", curName + "の実質公債費比率は" + cur.d + "%です。" + bodyD, true, dTrendPhrase, ka7TrendPhrase) + KK_CROSSCHECK_CAVEAT;
    }
    if ((code === "ka3" || code === "ka1" || code === "ka6") && cur.u != null) {
      ...
      if (code === "ka3") {
        ...
        if (!uHighRec && !highRec) { judgeU = "軽め"; bodyU = "<strong style='color:#3a9970;'>[財政]</strong>将来への借金・<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化、どちらの面から見ても軽い状態です。"; }
          ...
          judgeU = "軽め";
          var ka1TrendB = kkMetricTrend(entry, "ka1", entry ? entry.ka1 : null);
        ...
        else { judgeU = "軽め"; bodyU = "<strong style='color:#c0623a;'>[公会計]</strong>資産規模・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも小さめです。資産の規模と将来世代への負担のバランスは取れている状態です。"; }
        heading = "🏛️ 総務省公式の組み合わせ分析";
      } else {
        kkLabel = "住民一人当たり行政コスト"; unit = "万円";
        if (!uHighRec && !highRec) { judgeU = "軽め"; bodyU = "<strong style='color:#c0623a;'>[公会計]</strong>行政コスト・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも軽めです。コンパクトな運営と言えます。"; }
      ...
      function getUValRec(i){ return i===0 ? cur.u : cur["u_r"+i]; }
      var uTrendPhraseRec = withTrendMeaning("u", trendSincePhrase(getUValRec, 5, 6));
      function getKkValRec(i){ return i===0 ? entry[code] : entry[code+"_r"+i]; }
      var kkTrendPhraseRec = entry ? withTrendMeaning(code, trendSincePhrase(getKkValRec, 4, 5)) : null;
      return kkCrossBox(heading, "将来負担比率", cur.u+"%", judgeU, kkLabel, entryV+unit, highRec?"中央値より高め":"中央値より低め", curName + "の将来負担比率は" + cur.u + "%です。" + bodyU, true, uTrendPhraseRec, kkTrendPhraseRec) + KK_CROSSCHECK_CAVEAT;
    }
    return "";
  }
  var KK_META = {
