...
/* ===== 公会計機能 ===== */
  ...
  var KK_CROSSCHECK_CAVEAT = "<div style='background:rgba(160,139,232,0.08);border-radius:10px;padding:12px 14px;margin-top:10px;'>" +
    ...
    "</div>";
  function kkCrossBox(heading, zaiLabel, zaiVal, zaiJudge, kkLabel, kkVal, kkJudge, bodyText, reverse) {
  ...
  function kkReciprocalCross(code, entryV, isPref, entry) {
    ...
    if (code === "ka4" && cur.sfs && cur.sfs > 0) {
      ...
      return kkCrossBox("🔗 財政と合わせて見ると", "すぐ使える貯金", ratioRec.toFixed(1)+"%", judgeRec, "自己資本の厚み", entryV+"%", ka4HighRec?"中央値より高め":"中央値より低め", curName + "の財政調整基金は標準財政規模の" + ratioRec.toFixed(1) + "%です。" + bodyRec, true) + KK_CROSSCHECK_CAVEAT;
    }
    if (code === "ka7" && cur.d != null) {
      ...
      return kkCrossBox("🔗 財政と合わせて見ると", "毎年の返済負担", cur.d+"%", judgeD, "借金残高そのもの", entryV+"万円", ka7HighRec?"中央値より高め":"中央値より低め", curName + "の実質公債費比率は" + cur.d + "%です。" + bodyD, true) + KK_CROSSCHECK_CAVEAT;
    }
    if ((code === "ka3" || code === "ka1" || code === "ka6") && cur.u != null) {
      ...
      return kkCrossBox(heading, "将来負担比率", cur.u+"%", judgeU, kkLabel, entryV+unit, highRec?"中央値より高め":"中央値より低め", curName + "の将来負担比率は" + cur.u + "%です。" + bodyU, true) + KK_CROSSCHECK_CAVEAT;
    }
    return "";
  }
  var KK_META = {
