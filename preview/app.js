(function() {
  ...
  function openD(key, skipPush) {
    ...
    if (key === "reserve" && cur && cur.sfs && cur.sfs > 0) {
      ...
      topSummaryHtml += "<br><br><strong style='color:"+judgeColor+";'>"+curName+"の財政調整基金は標準財政規模の"+ratio.toFixed(1)+"%で、"+judge+"です。</strong>";
      ...
      if (rHist.length >= 2) {
        ...
        if (rRatio >= 1.5) {
          topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:15px;'>\u{1F4C8} "+curName+"の財政調整基金は過去平均（"+rAvg.toFixed(1)+"億円）より大きく増えています。国からの臨時交付金や、大型事業の先送りによる積立増加の可能性があります。</span>";
        } else if (rRatio <= 0.5) {
          topSummaryHtml += "<br><span style='color:#c04030;font-size:15px;'>\u{1F4C9} "+curName+"の財政調整基金は過去平均（"+rAvg.toFixed(1)+"億円）より大きく減っています。災害対応や大型事業への取り崩しがあった可能性があります。</span>";
        }
      }
      if (ratio < rb.lo && cur.x > 95) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>\u26A0\uFE0F 貯金が少なめ＋固定費も重い→緊急時に回せるお金が限られやすい状態です</span>";
      else if (ratio < rb.lo && cur.u > 100) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>\u26A0\uFE0F 貯金が少なめ＋将来への借金も重い→今の備えと将来の返済の両方に課題があります</span>";
      else if (ratio < rb.lo && cur.f < 0.5) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>\u26A0\uFE0F 貯金が少なめ＋自力収入も乏しい→大きな支出があったときの余力が限られます</span>";
      else if (ratio < rb.lo) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>\u26A0\uFE0F 緊急時に備える積立が課題になりやすい水準です</span>";
      else if (ratio < rb.mid && cur.x > 95) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>\u26A0\uFE0F 貯金はやや少なめ＋固定費も重い→固定費の削減と積立増加が同時に課題です</span>";
      else if (ratio < rb.mid) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>\u26A0\uFE0F もう少し積み増せると緊急時の備えとして安心できる水準です</span>";
      else if (ratio >= rb.hi && cur.u <= 0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>\u2728 貯金が多め＋将来への借金もない→備えも負債もバランスの良い状態です</span>";
      else if (ratio >= rb.hi && cur.x < 88) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>\u2728 貯金が多め＋固定費も軽い→財政に余裕があり、危機対応力の高い状態です</span>";
      else if (ratio >= rb.hi && cur.d > 25) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>\u26A0\uFE0F ※歳出が極端に削減された自治体（財政再生団体など）は、この比率が高く見える場合があります。<br>"+curName+"は実質公債費比率が"+cur.d+"%と非常に高く、この基金の多くは借金返済や不測の事態への積立です。財政状況は深刻です。</span>";
      else if (ratio >= rb.hi) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>\u2728 いざというときの「自治体の貯金」として機能しやすい水準です</span>";
...
if (key === "growth" && cur && cur.pop) {
      ...
      topSummaryHtml += "<br><br><strong style='color:"+popColor+";'>"+curName+"の人口は"+popStr+"人（令和7年1月1日時点）。前年比"+gSign+cur.g+"%です。</strong>";
      if (cur.pop < 3000) {
        topSummaryHtml += "<br><span style='color:#b8860b;font-size:15px;'>⚠️ "+curName+"は人口が少ない（"+popStr+"人）ため、少数の転入・転出だけでも増減率が大きく振れやすい点にご注意ください。</span>";
      }
      if (cur.g >= 0 && cur.f >= 0.7) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 人口が増えている＋財政力も安定→人が集まることで税収も増え、好循環が生まれやすい状態です</span>";
      else if (cur.g >= 0 && cur.ch >= 100) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 人口が増えている＋子どもへの投資も手厚い→子育て環境の充実が若い世代を引き寄せている可能性があります</span>";
      else if (cur.g >= 0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 人口が増加中→住民が増えれば税収も増え、財政の安定にもつながります。この流れを維持したいところです</span>";
      else if (cur.g < -1.0 && cur.x > 95) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 人口が急減している＋固定費が重い→税収が減るのに支出が固定化、財政悪化が加速しやすい危険な組み合わせです</span>";
      else if (cur.g < -1.0 && cur.f < 0.5) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 人口が急減している＋財政力も弱い→人口減少が税収減を招き、財政がじわじわ悪化するリスクが高い状態です</span>";
      else if (cur.g < -0.5) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 人口の減少が続いている→住民1人あたりの行政コストが上がり、財政を圧迫しやすくなります。定住促進策が急務です</span>";
      else topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 人口はやや減少傾向→緩やかな減少でも長期的には財政に影響します。子育て支援・移住促進が重要です</span>";
    }
    // 各項目の判定メッセージ
    if (key === "health" && cur) {
      ...
      topSummaryHtml += "<div style='background:rgba(232,160,150,0.1);border:1.5px solid rgba(232,160,150,0.3);border-radius:10px;padding:10px 12px;margin-top:10px;'><strong style='color:"+hc+";'>"+curName+"の総合スコアは"+h2+"点で、"+hj+"です。</strong>" + (curTypeLabel ? " <span style='color:#8070c0;font-size:12px;'>（"+curTypeLabel+"）</span>" : "") + "</div>";
    ...
    if (key === "debt" && cur) {
      ...
      topSummaryHtml += "<br><br><strong style='color:"+dc2+";'>"+curName+"の実質公債費比率は"+cur.d+"%で、"+dj+"。</strong>";
      if (cur.d > 18 && cur.f < 0.5) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 借金返済が重い＋自力収入が乏しい→返済で手一杯なのに稼ぐ力もない、最も厳しい二重苦です</span>";
      else if (cur.d > 18 && cur.x > 95) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 借金返済が重い＋固定費も硬直化→お金の出口がふさがれており、新しい政策に回せる余地がほぼありません</span>";
      else if (cur.d > 18) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 借金返済の負担が大きい→収入の多くが返済に消えており、住民サービスへの影響が出やすい状態です</span>";
      else if (cur.d < 10 && cur.f >= 0.7 && cur.x < 88) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 借金返済が軽い＋自力収入も豊か＋固定費も適正→返済の心配がなく政策投資もできる、理想的な状態です</span>";
      else if (cur.d < 10 && cur.f >= 0.7) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 借金返済が軽い＋自力収入も豊か→返済の心配がなく、新しい政策にも積極的に投資できる状態です</span>";
      else if (cur.d < 10 && reserveIsAmple(cur, isPrefView)) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 借金返済が軽い＋貯金も十分（標準財政規模比で多め）→いざというときの備えもあり、とても健全な財政です</span>";
      else if (cur.d < 10 && cur.x > 95) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 借金返済は軽いが固定費が重い→返済の心配はないものの、人件費・社会保障費が財政を圧迫しています</span>";
      else if (cur.d < 10) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 借金返済の負担が軽く、財政に余裕があります。この水準を維持できると理想的です</span>";
      else if (cur.d < 18 && cur.x > 95) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 返済負担は標準的だが固定費が重い→借金は普通でも固定費に圧迫されており、政策の自由度が低い状態です</span>";
      else if (cur.d < 18 && cur.f >= 0.7) topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 返済負担は標準的＋自力収入も安定→大きな心配はありませんが、借入残高の推移は引き続き注視を</span>";
      else topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 返済負担がやや重め＋自力収入が少ない→交付税頼みになりやすく、国の制度変更の影響を受けやすい状態です</span>";
    }
    if (key === "fiscalPower" && cur) {
      ...
      topSummaryHtml += "<br><br><strong style='color:"+fc2+";'>"+curName+"の財政力指数は"+cur.f.toFixed(2)+"で、"+fj+"です。</strong>";
      if (cur.f < 0.5 && cur.x > 95) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 自力収入が乏しい＋固定費が重い→稼げないのにお金が出ていく一方、最も身動きが取りにくい状態です</span>";
      else if (cur.f < 0.5 && cur.u > 100) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 自力収入が乏しい＋将来への借金も重い→今も苦しく将来も重荷を背負っており、構造的な改革が必要です</span>";
      else if (cur.f < 0.5) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 自力で稼ぐ力が弱い→国からの交付税に大きく依存しており、国の財政事情に左右されやすい状態です</span>";
      else if (cur.f >= 1.0 && cur.x < 88) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 自力収入が豊か＋固定費も軽い→収入も支出もバランス良く、政策の自由度が高い理想的な財政です</span>";
      else if (cur.f >= 1.0 && reserveIsAmple(cur, isPrefView)) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 自力収入が豊か＋貯金も十分（標準財政規模比で多め）→財政力があり備えもある、とても安定した状態です</span>";
      else if (cur.f >= 1.0 && cur.x > 95) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 自力収入は豊かだが固定費が重い→稼ぐ力はあるのに固定費に消えており、政策投資の余地が限られています</span>";
      else if (cur.f >= 1.0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 国からの交付税に頼らず自立した財政→住民サービスを自分たちの収入でまかなえる、強い自治体です</span>";
      else if (cur.f >= 0.7 && cur.x < 95) topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 財政力は安定＋固定費も許容範囲→大きな問題はなく、引き続きこの水準の維持が目標です</span>";
      else topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 交付税依存が高め→自主財源を増やす取り組み（企業誘致・定住促進など）が長期的な課題です</span>";
    }
    if (key === "flex" && cur) {
      var xj = cur.x<88?"財政に弾力性がある状態":cur.x<95?"標準的な水準":"硬直化した状態";
      var xc2 = cur.x<88?"#6dcfad":cur.x<95?"#7bb8e8":"#f0876a";
      topSummaryHtml += "<br><br><strong style='color:"+xc2+";'>"+curName+"の経常収支比率は"+cur.x.toFixed(1)+"%で、"+xj+"です。</strong>";
      if (cur.x >= 100) {
        topSummaryHtml += "<br><span style='color:#a02030;font-size:15px;'>🚨 "+curName+"は経常収支比率が100%を超えているため、経常的な収入だけでは経常的な支出をまかないきれていない状態です。</span>";
      }
      if (cur.x > 95 && cur.f < 0.5) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 固定費が重い＋自力収入も乏しい→収入が少ないのに出費が固定化、新しいことに一切お金を使えない状態です</span>";
      else if (cur.x > 95 && cur.r < 10) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 固定費が重い＋貯金も少ない→日常の支出でギリギリで、いざというときの備えもない、じわじわ危ない状態です</span>";
      else if (cur.x > 95 && cur.u > 100) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 固定費が重い＋将来への借金も多い→今の家計も苦しく将来の返済も重い、二重の重荷を抱えています</span>";
      else if (cur.x > 95) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 固定費が重く硬直化→人件費・社会保障費・借金返済が収入の大半を占め、政策の自由度が低い状態です</span>";
      else if (cur.x < 88 && cur.r > 20) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 固定費が軽い＋貯金も十分→支出に余裕があり備えもある、財政運営の理想的な姿です</span>";
      else if (cur.x < 88 && cur.f >= 1.0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 固定費が軽い＋自力収入も豊か→稼いでいて使い方も健全、新しい政策に積極投資できる状態です</span>";
      else if (cur.x < 88) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 固定費が適正で財政に弾力性がある→急な支出や新しい施策にも対応しやすい、健全な状態です</span>";
      else if (cur.x < 95 && cur.f >= 0.7) topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 固定費は標準的＋財政力も安定→大きな問題はありませんが、固定費が増えすぎないよう注視が必要です</span>";
      else topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 固定費は標準的な水準→大きな問題はありませんが、95%を超えると硬直化します。推移を注視しましょう</span>";
    }
    if (key === "future" && cur) {
      ...
      topSummaryHtml += "<br><br><strong style='color:"+uc2+";'>"+curName+"の将来負担比率は"+uv+"で、"+ul2+"です。</strong>";
      if (cur.u > 100 && cur.r < 10) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 将来への借金が重い＋貯金も少ない→今も苦しく将来も重荷、借金まみれで備えもない綱渡りの状態です</span>";
      else if (cur.u > 100 && cur.x > 95) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 将来への借金が重い＋毎年の固定費も重い→過去の借金に縛られながら今も出費が固定化、構造改革が急務です</span>";
      else if (cur.u > 100 && cur.f < 0.5) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 将来への借金が重い＋自力収入も乏しい→借金を返す力が弱く、長期的に財政が悪化するリスクがあります</span>";
      else if (cur.u > 100) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 将来世代への借金が重い→今の住民が使ったお金を将来世代が返す構図で、世代間の公平性が問われます</span>";
      else if (cur.u <= 0 && reserveIsAmple(cur, isPrefView)) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 将来への借金がない＋貯金も十分（標準財政規模比で多め）→将来世代に負担を残さず、備えもある。財政の優等生です</span>";
      else if (cur.u <= 0 && cur.f >= 1.0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 将来への借金がない＋自力収入も豊か→借金ゼロで稼ぐ力もある、非常に健全な財政状態です</span>";
      else if (cur.u <= 0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 将来世代への負担がない→過去の借金を着実に返し終えており、次世代に重荷を残さない健全な状態です</span>";
      else if (cur.u < 100 && reserveIsAmple(cur, isPrefView)) topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 将来への負担は一定あるが貯金（標準財政規模比で多め）で備えもある→心配しすぎる必要はないが、借入残高の推移は要注視です</span>";
      else if (cur.u < 100 && cur.x > 95) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 将来への借金がやや重い＋固定費も重い→返済余力が乏しく、借入残高の削減が急務です</span>";
      else topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 将来への借金がやや重め→公共施設の維持費・更新費も増える中、新規借入の抑制が課題です</span>";
    ...
    if (key === "education" && cur && cur.edu!=null) {
      ...
      topSummaryHtml += "<br><br><strong style='color:#a08be8;'>"+curName+"の教育費比率は"+cur.edu.toFixed(1)+"%で、"+eduUnit+"の"+ej+"（中央値"+eduMed+"%）。</strong>";
      topSummaryHtml += "<br><span style='color:#6b5b95;font-size:14px;'>\u2139\uFE0F この数字は高い・低いが、そのまま良い・悪いを意味しません。高いのは教育を重視している場合もあれば、学校施設の老朽化対応や小規模校の維持で費用がかさんでいる場合もあります。都道府県は高校を持つため、市区町村より高く出ます。</span>";
      ...
      if (eduHist.length >= 2) {
        ...
        if (eduRatio <= 0.7) {
          topSummaryHtml += "<br><span style='color:#3070b8;font-size:15px;'>📉 "+curName+"の教育費比率は過去平均（"+eduAvg.toFixed(1)+"%）より大きく下がっています。学校施設整備の完了、または災害復旧費など他の歳出が増えたことで相対的に比率が下がった可能性があります。</span>";
          eduCaveatFired = true;
        } else if (eduRatio >= 1.3) {
          topSummaryHtml += "<br><span style='color:#b8860b;font-size:15px;'>📈 "+curName+"の教育費比率は過去平均（"+eduAvg.toFixed(1)+"%）より大きく上がっています。学校施設整備などの大型事業、または他の歳出が減ったことで相対的に比率が上がった可能性があります。</span>";
          eduCaveatFired = true;
        }
      }
      if (cur.edu >= 10 && cur.ch != null && cur.ch >= 100) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 教育費が多い＋子ども1人あたりの投資も手厚い→お金も手間もかけて子どもを育てる姿勢が数字に表れています</span>";
      else if (cur.edu >= 10 && cur.x > 95) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 教育には力を入れているが固定費も重い→苦しい財政の中でも教育を優先している姿勢は評価できますが、持続可能性に注意が必要です</span>";
      else if (cur.edu >= 10 && cur.f < 0.5) topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 財政力は弱いが教育には力を入れている→苦しい中でも未来への投資を優先している姿勢が見えます</span>";
      else if (cur.edu >= 10 && !eduCaveatFired) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 教育への支出が手厚い→子どもの学びへの投資は将来の地域力につながる、前向きな財政です</span>";
      else if (cur.edu < 6 && cur.x > 95) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 教育費が少ない＋固定費が重い→固定費に圧迫されて教育への投資が削られている可能性があります</span>";
      else if (cur.edu < 6 && !eduCaveatFired) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 教育への支出が少なめ→財政的制約がある可能性がありますが、子どもへの投資は将来の税収にも影響します</span>";
      else if (!eduCaveatFired) topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 教育費は標準的な水準→引き続き子どもへの投資の量と質を確認していきましょう</span>";
    }
    if (key === "childInvest" && cur && cur.ch!=null) {
      ...
      topSummaryHtml += "<br><br><strong style='color:#a08be8;'>"+curName+"の子ども1人当たり投資額は"+cur.ch.toFixed(1)+"万円で、"+chUnit+"の"+cj2+"（中央値"+chMed+"万円）。</strong>";
      topSummaryHtml += "<br><span style='color:#6b5b95;font-size:14px;'>\u2139\uFE0F この数字は高い・低いが、そのまま良い・悪いを意味しません。子どもの人数で割った値なので、子どもが少ない自治体ほど大きく出ます。全国で最も高いのは974万円ですが、これは手厚いのではなく分母が小さいためです。</span>";
      var chCaveatFired = false;
      if (cur.pop && cur.pop < 3000 && cur.ch > 250) {
        topSummaryHtml += "<br><span style='color:#b8860b;font-size:15px;'>⚠️ "+curName+"は人口が少ない（"+cur.pop.toLocaleString()+"人）ため、子どもの人数自体が少なく、1人当たりで計算すると数値が実態以上に大きくなります。</span>";
        chCaveatFired = true;
      }
      ...
      if (chHist.length >= 2) {
        ...
        if (chRatio <= 0.7) {
          topSummaryHtml += "<br><span style='color:#3070b8;font-size:15px;'>📉 "+curName+"の投資額は過去平均（"+chAvg.toFixed(1)+"万円）より大きく下がっています。学校施設整備の完了、または他の要因（人口変動など）が影響している可能性があります。</span>";
          chCaveatFired = true;
        } else if (chRatio >= 1.3) {
          topSummaryHtml += "<br><span style='color:#b8860b;font-size:15px;'>📈 "+curName+"の投資額は過去平均（"+chAvg.toFixed(1)+"万円）より大きく上がっています。学校施設整備などの大型事業、または人口変動が影響している可能性があります。</span>";
          chCaveatFired = true;
        }
      }
      if (cur.ch >= 120 && cur.edu >= 10) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 子ども1人への投資が手厚い＋教育費比率も高い→量も割合も子どもに手をかけている、子育て支援に本気な自治体です</span>";
      else if (cur.ch >= 120 && cur.g >= 0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 子どもへの投資が手厚い＋人口も増えている→子育て環境の充実が人口流入につながっている好循環です</span>";
      else if (cur.ch >= 120 && !chCaveatFired) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 子ども1人あたりへの投資が手厚い→将来の地域を担う子どもへの投資は、長期的に見て最も大切な支出です</span>";
      else if (cur.ch < 70 && cur.x > 95) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 子どもへの投資が少ない＋固定費が重い→固定費に圧迫されて将来世代への投資が削られている構図です</span>";
      else if (cur.ch < 70 && cur.g < -0.5) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 子どもへの投資が少ない＋人口も減少中→投資不足が子育て世代の流出を招いている可能性があります</span>";
      else if (cur.ch < 70 && !chCaveatFired) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 子どもへの投資が少なめ→保育・教育・医療費補助などの充実が、将来の定住促進にもつながります</span>";
      else if (!chCaveatFired) topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 子どもへの投資は標準的→近隣自治体と比較しながら、子育て世代に選ばれる地域づくりを目指したいところです</span>";
    }
    if (key === "budget" && cur && cur.eo && cur.ei) {
      ...
      topSummaryHtml += "<br><br><strong style='color:"+bc2+";'>"+curName+"の歳出は"+cur.eo.toLocaleString()+"億円、歳入は"+cur.ei.toLocaleString()+"億円で、"+bj+"。</strong>";
      ...
      if (eoHist.length >= 2) {
        ...
        if (eoRatio >= 1.3) {
          topSummaryHtml += "<br><span style='color:#b8860b;font-size:15px;'>📈 "+curName+"の歳出は過去平均（"+eoAvg.toFixed(1)+"億円）より大きく増えています。災害復旧や大型施設整備など、一時的な要因が影響している可能性があります。</span>";
        } else if (eoRatio <= 0.7) {
          topSummaryHtml += "<br><span style='color:#3070b8;font-size:15px;'>📉 "+curName+"の歳出は過去平均（"+eoAvg.toFixed(1)+"億円）より大きく減っています。前年度の大型事業や特別な支出が終了した可能性があります。</span>";
        }
      }
      if (cur.ei < cur.eo && cur.x > 95) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 収支が赤字＋固定費も重い→支出が収入を超えているのに固定費が大半を占め、改善の余地がほぼない最も厳しい状態です</span>";
      else if (cur.ei < cur.eo && cur.r < 10) topSummaryHtml += "<br><span style='color:#a02030;font-size:16px;'>🚨 収支が赤字＋貯金も少ない→赤字を補う備えもなく、財政の持続可能性に黄信号です</span>";
      else if (cur.ei < cur.eo && cur.u > 100) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 収支が赤字＋将来への借金も重い→今も赤字で将来の返済も重く、収支改善が急務です</span>";
      else if (cur.ei < cur.eo) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 収入より支出が多い→財政調整基金や借入で補っている可能性があります。収支のバランス改善が課題です</span>";
      else if (cur.ei >= cur.eo && cur.x > 95) topSummaryHtml += "<br><span style='color:#c04030;font-size:16px;'>⚠️ 収支は均衡しているが固定費が大半を占めている→数字上は黒字でも人件費・返済・社会保障で身動きが取れない硬直した状態です</span>";
      else if (cur.ei >= cur.eo && cur.x > 88) topSummaryHtml += "<br><span style='color:#b8860b;font-size:16px;'>⚠️ 収支は均衡しているが固定費がやや重め→収支は安定しているものの、新しい政策への投資余地は限られています</span>";
      else if (cur.ei >= cur.eo && cur.f >= 1.0) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 収支が黒字＋財政力も高い＋固定費も適正→自力で稼ぎ支出も健全、最も理想的な財政運営です</span>";
      else if (cur.ei >= cur.eo && cur.r > 20) topSummaryHtml += "<br><span style='color:#2a8a6a;font-size:16px;'>✨ 収支が黒字＋貯金も十分＋固定費も適正→稼いでいて備えもある、とても安定した財政運営です</span>";
      else topSummaryHtml += "<br><span style='color:#3070b8;font-size:16px;'>📋 収支は均衡＋固定費も標準的→大きな問題はありませんが、社会保障費の増加など将来の支出増に備えた積立が重要です</span>";
    }
    document.getElementById("shDesc").innerHTML = topSummaryHtml.replace(/^(<br>)+/, "") + descHtml;
