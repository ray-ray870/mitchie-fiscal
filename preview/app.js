...
if (key === "growth" && cur && cur.pop) {
    ...
    if (key === "budget" && cur && cur.eo && cur.ei) {
      ...
      // 歳出のピーク・底の年度がコロナ対応期間（R2〜R4）と重なる場合の注記
      var budgetHistCountEarly = 0;
      for (var bhc=1; bhc<=4; bhc++){ if (cur["eo_r"+bhc]!=null) budgetHistCountEarly++; else break; }
      if (budgetHistCountEarly >= 1) {
        var eoValsEarly = [];
        for (var bhi=budgetHistCountEarly; bhi>=1; bhi--) { eoValsEarly.push(cur["eo_r"+bhi]); }
        eoValsEarly.push(cur.eo);
        var budgetYrsEarly = ["R1","R2","R3","R4"].slice(0,budgetHistCountEarly).concat(["R"+(budgetHistCountEarly+1)]);
        if (eoValsEarly.length >= 2) {
          var bPeakI=0, bTroughI=0;
          for (var bpi=1; bpi<eoValsEarly.length; bpi++){ if(eoValsEarly[bpi]>eoValsEarly[bPeakI]) bPeakI=bpi; if(eoValsEarly[bpi]<eoValsEarly[bTroughI]) bTroughI=bpi; }
          var bLastI = eoValsEarly.length-1;
          var bCovidSet = {"R2":1,"R3":1,"R4":1};
          if (bPeakI!==bLastI && bCovidSet[budgetYrsEarly[bPeakI]]) {
            topSummaryHtml += "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+budgetYrsEarly[bPeakI]+"年度の歳出が最も多い年です。新型コロナウイルス対応の時期と重なり、コロナ対応費が直接歳出に計上された影響が考えられます</div>";
          } else if (bTroughI!==bLastI && bCovidSet[budgetYrsEarly[bTroughI]]) {
            topSummaryHtml += "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+budgetYrsEarly[bTroughI]+"年度の歳出が最も少ない年です。新型コロナウイルス対応の時期と重なり、コロナ対応費が直接歳出に計上された影響が考えられます</div>";
          }
        }
      }
    ...
    var covidReasons = {
      reserve: "コロナ対応での取り崩し・積み増しの影響が考えられます",
      growth: "コロナ禍での転出入の変化の影響が考えられます",
      education: "GIGAスクール構想の端末整備などで支出が急増した時期と重なります",
      childInvest: "教育・子育て関連のコロナ対応費の影響が考えられます"
    };
    var trendYrLabelsCovid = covidReasons[key] ? (hasHistory ? yrs.map(function(y){ return y.replace("（最新）",""); }) : hasGrowthHistory ? growthYrLabels.slice(0, vals.length) : null) : null;
    if (trendYrLabelsCovid && vals.length >= 2) {
      ...
      if (validIdxCovid.length >= 2) {
        ...
        if (peakICovid !== lastValidCovid && covidYrsSet[trendYrLabelsCovid[peakICovid]]) {
          topSummaryHtml += "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+trendYrLabelsCovid[peakICovid]+"年度が最も高い年です。新型コロナウイルス対応の時期と重なり、"+covidReasons[key]+"</div>";
        } else if (troughICovid !== lastValidCovid && covidYrsSet[trendYrLabelsCovid[troughICovid]]) {
          topSummaryHtml += "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+trendYrLabelsCovid[troughICovid]+"年度が最も低い年です。新型コロナウイルス対応の時期と重なり、"+covidReasons[key]+"</div>";
        }
      }
    }
...
/* ===== 公会計機能 ===== */
  ...
  function kkOpenDetail(code, skipPush){
    ...
    if (kkVals.length >= 2 && kkVals.every(function(v){ return v != null; })) {
      ...
      // 行政コスト(ka6)・業務投資活動収支(ka8)のみ、コロナ対応費が直接出やすいため対象にする。
      var kkCovidReasons = {
        ka6: "コロナ対応費が直接計上された影響が考えられます",
        ka8: "コロナ対応費の収支への影響が考えられます"
      };
      if (kkCovidReasons[code] && kkPeakI !== kkLastI && kkCovidYrsSet[kkYrLabels[kkPeakI]]) {
        kkCovidNote = "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+kkYrLabels[kkPeakI]+"年度が最も高い年です。新型コロナウイルス対応の時期と重なり、"+kkCovidReasons[code]+"</div>";
      } else if (kkCovidReasons[code] && kkTroughI !== kkLastI && kkCovidYrsSet[kkYrLabels[kkTroughI]]) {
        kkCovidNote = "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+kkYrLabels[kkTroughI]+"年度が最も低い年です。新型コロナウイルス対応の時期と重なり、"+kkCovidReasons[code]+"</div>";
      }
