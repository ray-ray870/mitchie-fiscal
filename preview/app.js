...
if (key === "growth" && cur && cur.pop) {
    ...
    if (key === "debt" && cur) {
      ...
      if (KK && KK[curName] && KK[curName].ka7 != null) {
        ...
        if (!dHigh && !ka7High) {
          ...
          ka7Judge = "中央値より高め";
          linkMsgD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は軽めに見えますが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体は大きめです。返済期間を長く取っている可能性があります。";
        } else {
          ka7Judge = "中央値より低め";
          linkMsgD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は重めですが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体はそれほど大きくありません。返済期間を短く設定している可能性があります。";
        }
        topSummaryHtml += kkCrossBox("🔗 公会計と合わせて見ると",
    ...
    if (key === "future" && cur) {
      ...
      if (KK && KK[curName] && KK[curName].ka3 != null) {
        ...
        if (!ka3Near) {
          ...
          else if (!uHighF && ka3High) { ka3Judge = "中央値より高め"; ka3Body = "<strong style='color:#3a9970;'>[財政]</strong>将来への借金は軽めですが、<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化は進んでいます。①必要な投資が行われず、老朽化対策が先送りにされている可能性、②既存施設を活用して財政負担を抑えている可能性、が考えられます。"; }
          ...
          topSummaryHtml += kkCrossBox("🏛️ 総務省公式の組み合わせ分析（将来負担比率×老朽化率）",
      ...
      if (KK && KK[curName] && KK[curName].ka1 != null) {
        ...
        if (!ka1Near4) {
          ...
          else { ka1Judge4 = "中央値より低め"; ka1Body = "<strong style='color:#c0623a;'>[公会計]</strong>資産規模・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも小さめです。大きな整備は行っていませんが、身の丈に合った運営とも言えます。"; }
          topSummaryHtml += kkCrossBox("🏛️ 総務省公式の組み合わせ分析（住民一人当たり資産×将来負担）",
            "将来への借金", cur.u+"%", uHighF?"重め":"軽め",
      ...
      if (KK && KK[curName] && KK[curName].ka6 != null) {
        ...
        if (!ka6Near4) {
          ...
          topSummaryHtml += kkCrossBox("🏛️ 総務省公式の組み合わせ分析（住民一人当たり行政コスト×将来負担）",
            "将来への借金", cur.u+"%", uHighF?"重め":"軽め",
  ...
  /* ===== 全国のみっちー ===== */
  function nkDistHtmlFromKeys(keys, title){
  ...
  function nkDistHtml(){
    if (!DB) return "";
    return nkDistHtmlFromKeys(Object.keys(DB), "全国の分布");
  }

  function nkPrefRow(list, idx, mineSet, natRank){
    var o = list[idx];
    ...
    return "<div class='nk-pref-row"+(mine?" mine":"")+"'>"+
      ...
      "<span class='nk-pref-name' data-pref='"+o.k+"'>"+o.k+(mine?" <span class='nk-mine-chip'>Myみっちー</span>":"")+natRankHtml+"</span>"+
  ...
  function nkBlockRankHtml(regionIdx){
    ...
    var fullList = nkPrefList(); // 全国順位（スコア降順）
    var natRankMap = {};
    fullList.forEach(function(o, i){ natRankMap[o.k] = i + 1; });
    var blockList = fullList.filter(function(o){ return region.prefs.indexOf(o.k) !== -1; });
    if (!blockList.length) return "<p style='font-size:13px;color:#999;text-align:center;padding:10px 0;'>データがありません</p>";
    var reg = mmLoad();
    var mineSet = {};
    if (reg && reg.pref) mineSet[reg.pref] = true;
    var html = nkDistHtmlFromKeys(region.prefs, region.name+"の分布");
    html += "<div class='nk-divider'></div>";
    html += "<p style='font-size:14px;font-weight:700;color:#3a2a6e;margin:10px 0 4px;'>"+region.name+"ランキング（"+blockList.length+"県中）</p>";
    html += NK_ZONE_LEGEND;
    for (var i=0; i<blockList.length; i++){
      html += nkPrefRow(blockList, i, mineSet, natRankMap[blockList[i].k]);
    }
    return html;
  }
  ...
  function nkRenderCard(){
    ...
    if (!body.dataset.prefClickBound) {
      ...
      body.addEventListener("click", function(e){
        ...
        if (nameEl) { nkGoToPref(nameEl.getAttribute("data-pref")); }
      });
    }
    ...
    regionBtns.forEach(function(btn){
      btn.addEventListener("click", function(){
        ...
        if (alreadyActive) {
          ...
          regionListEl.innerHTML = nkBlockRankHtml(idx);
        }
      });
    });
  }
...
/* ===== 公会計機能 ===== */
  ...
  function kkCrossBox(heading, zaiLabel, zaiVal, zaiJudge, kkLabel, kkVal, kkJudge, bodyText) {
    return "<div style='background:#e8f7f0;border:1px solid #6dcfad55;border-radius:12px;padding:12px 14px;margin-top:10px;'>" +
      "<div style='font-size:13px;font-weight:700;color:#3a9970;margin-bottom:8px;'>" + heading + "</div>" +
      "<div style='display:flex;gap:8px;margin-bottom:8px;'>" +
        "<div style='flex:1;background:white;border-radius:10px;padding:8px 10px;text-align:center;'>" +
          "<div style='font-size:10px;color:#3a9970;font-weight:700;margin-bottom:2px;'>[財政] " + zaiLabel + "</div>" +
          "<div style='font-size:15px;font-weight:700;color:#2a2a3a;'>" + zaiVal + "</div>" +
          "<div style='font-size:10px;color:#888;'>" + zaiJudge + "</div>" +
        "</div>" +
        "<div style='flex:1;background:white;border-radius:10px;padding:8px 10px;text-align:center;'>" +
          "<div style='font-size:10px;color:#c0623a;font-weight:700;margin-bottom:2px;'>[公会計] " + kkLabel + "</div>" +
          "<div style='font-size:15px;font-weight:700;color:#2a2a3a;'>" + kkVal + "</div>" +
          "<div style='font-size:10px;color:#888;'>" + kkJudge + "</div>" +
        "</div>" +
      "</div>" +
      "<div style='font-size:13px;color:#3a5a4a;line-height:1.7;'>" + bodyText + "</div>" +
    "</div>";
  }
  // 公会計側のポップアップから見た、財政側とのクロスチェック（財政タブ側と同じロジックの逆方向）
  function kkReciprocalCross(code, entryV, isPref) {
    if (!cur) return "";
    if (code === "ka4" && cur.sfs && cur.sfs > 0) {
      ...
      return kkCrossBox("🔗 財政と合わせて見ると", "すぐ使える貯金", ratioRec.toFixed(1)+"%", judgeRec, "自己資本の厚み", entryV+"%", ka4HighRec?"中央値より高め":"中央値より低め", curName + "の財政調整基金は標準財政規模の" + ratioRec.toFixed(1) + "%です。" + bodyRec);
    }
    if (code === "ka7" && cur.d != null) {
      ...
      else if (!dHighRec && ka7HighRec) { judgeD = "軽め"; bodyD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は軽めに見えますが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体は大きめです。返済期間を長く取っている可能性があります。"; }
      else { judgeD = "重め"; bodyD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は重めですが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体はそれほど大きくありません。返済期間を短く設定している可能性があります。"; }
      return kkCrossBox("🔗 財政と合わせて見ると", "毎年の返済負担", cur.d+"%", judgeD, "借金残高そのもの", entryV+"万円", ka7HighRec?"中央値より高め":"中央値より低め", curName + "の実質公債費比率は" + cur.d + "%です。" + bodyD);
    }
    if ((code === "ka3" || code === "ka1" || code === "ka6") && cur.u != null) {
      ...
      if (code === "ka3") {
        ...
        else if (!uHighRec && highRec) { judgeU = "軽め"; bodyU = "<strong style='color:#3a9970;'>[財政]</strong>将来への借金は軽めですが、<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化は進んでいます。①必要な投資が行われず、老朽化対策が先送りにされている可能性、②既存施設を活用して財政負担を抑えている可能性、が考えられます。"; }
        ...
        heading = "🏛️ 総務省公式の組み合わせ分析（将来負担比率×老朽化率）";
      } else if (code === "ka1") {
        ...
        else { judgeU = "軽め"; bodyU = "<strong style='color:#c0623a;'>[公会計]</strong>資産規模・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも小さめです。大きな整備は行っていませんが、身の丈に合った運営とも言えます。"; }
        heading = "🏛️ 総務省公式の組み合わせ分析（住民一人当たり資産×将来負担）";
      } else {
        ...
        heading = "🏛️ 総務省公式の組み合わせ分析（住民一人当たり行政コスト×将来負担）";
      }
      return kkCrossBox(heading, "将来への借金", cur.u+"%", judgeU, kkLabel, entryV+unit, highRec?"中央値より高め":"中央値より低め", curName + "の将来負担比率は" + cur.u + "%です。" + bodyU);
    }
    return "";
  }
  var KK_META = {
  ...
  function kkOpenDetail(code, skipPush){
    ...
    if (kkVals.length >= 2 && kkVals.every(function(v){ return v != null; })) {
      ...
      document.getElementById("shTop").innerHTML = cmpHtml + kkCovidNote + kkReciprocalCross(code, entry[code], isPref);
      ...
      document.getElementById("shTop").innerHTML = cmpHtml + kkReciprocalCross(code, entry[code], isPref);
      document.getElementById("spSvg").innerHTML = "";
      document.getElementById("spSvg").style.height = "0px";
      document.getElementById("spLabels").innerHTML = "";
