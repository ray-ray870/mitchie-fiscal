  ...
  function openD(key, skipPush) {
    ...
    if (key === "furusato") {
      ...
      if (DB && cur) {
        if (isPrefViewF) {
          ...
          if (myRankPF) {
            ...
            rankHtmlF = "<div style='display:inline-flex;align-items:center;gap:6px;background:#6dcfad18;border:1.5px solid #6dcfad44;border-radius:12px;padding:6px 14px;margin:6px 0 18px;font-size:14px;font-weight:700;color:#2a8a6a;'>" +
          ...
          if (myRankNF) {
            ...
            linesF += "<div style='display:inline-flex;align-items:center;gap:6px;background:#6dcfad18;border:1.5px solid #6dcfad44;border-radius:12px;padding:6px 14px;margin:0 0 6px;font-size:14px;font-weight:700;color:#2a8a6a;'>" + (medalNF?medalNF+" ":"") + "全国 <span style='font-size:17px;'>" + myRankNF + "位</span>/" + natListF.length + "自治体中</div>";
          }
          if (myRankPrefF) {
            var medalPF2 = medalFor(myRankPrefF, false);
            linesF += "<div style='display:inline-flex;align-items:center;gap:6px;background:#6dcfad18;border:1.5px solid #6dcfad44;border-radius:12px;padding:6px 14px;margin:0;font-size:14px;font-weight:700;color:#2a8a6a;'>" + (medalPF2?medalPF2+" ":"") + cur.p + "内 <span style='font-size:17px;'>" + myRankPrefF + "位</span>/" + prefListF2.length + "自治体中</div>";
          }
      ...
      var topSummaryHtmlF = "<div style='background:"+mainColor+"14;border:1px solid "+mainColor+"55;border-radius:12px;padding:12px 14px;'>" +
        ...
        "</div>" +
        "<div style='font-size:14px;color:#3a3a4a;line-height:1.7;'><span style='color:"+mainColor+";font-weight:700;'>"+curName+"</span>は、"+relText+"、ふるさと納税で<span style='color:"+mainColor+";font-weight:700;'>"+mainTerm+"</span>です。</div>";
      if (fuH.length >= 2 && fuH[0] > 0) {
        var ratioF = fuH[fuH.length-1] / fuH[0];
        if (ratioF >= 1.5) topSummaryHtmlF += "<div style='font-size:13px;color:#5a5a7a;margin-top:6px;'>📈 受入額はこの8年でおよそ"+ratioF.toFixed(1)+"倍に増加しており、右肩上がりの傾向が続いています</div>";
        else if (ratioF <= 0.67) topSummaryHtmlF += "<div style='font-size:13px;color:#5a5a7a;margin-top:6px;'>📉 受入額はこの8年でおよそ"+(1/ratioF).toFixed(1)+"分の1に減少しており、減少傾向が続いています</div>";
        else topSummaryHtmlF += "<div style='font-size:13px;color:#5a5a7a;margin-top:6px;'>➡️ 受入額はこの8年で大きな変化はなく、横ばい傾向です</div>";
      }
      topSummaryHtmlF += "<div style='font-size:11px;color:#9090a8;margin-top:8px;'>※受入額は返礼品等の経費を引く前の総額です</div>";
      topSummaryHtmlF += "</div>";

      document.getElementById("shTop").innerHTML = rankHtmlF + topSummaryHtmlF;

      // グラフ（受入額・住民税控除額の2本線。歳出／歳入と同じ描き方）
      function fmtFuruShort(v){ return v>=10000 ? (Math.round(v/1000)/10)+"億" : Math.round(v)+"万"; }
      var Wf=300, Hf=100, Pf=14;
      var allVf = fuH.concat(fkH);
      var mnf=Math.min.apply(null,allVf), mxf=Math.max.apply(null,allVf);
      var rngf = Math.max(mxf-mnf, mxf*0.15) || 1;
      mnf = mnf - rngf*0.1; mxf = mxf + rngf*0.1; rngf = mxf-mnf;
      var Ptopf=22, Pbottomf=26;
      function pxf(i){ return Pf+(i/(fuH.length-1))*(Wf-Pf*2); }
      function pyf(v){ return Hf-Pbottomf-((v-mnf)/rngf)*(Hf-Ptopf-Pbottomf); }
      function mkLineF(arr){ var l=null; for(var j=0;j<arr.length;j++){ if(arr[j]==null) continue; l = l===null ? ("M"+pxf(j)+","+pyf(arr[j])) : (l+" L"+pxf(j)+","+pyf(arr[j])); } return l||""; }
      var fuLine = mkLineF(fuH), fkLine = mkLineF(fkH);
      var dotsF = "";
      var maxIdxF = Math.max(fuH.length, fkH.length);
      for (var kf=0; kf<maxIdxF; kf++){
        var isLastF = kf===maxIdxF-1;
        var fuV=fuH[kf], fkV=fkH[kf];
        var fuOnTop = (fuV!=null && fkV!=null) ? (fuV>=fkV) : true;
        if (fuV != null) {
          dotsF += "<circle cx='"+pxf(kf)+"' cy='"+pyf(fuV)+"' r='"+(isLastF?5:3)+"' fill='"+(isLastF?"#6dcfad":"white")+"' stroke='#6dcfad' stroke-width='2'/>";
          if (fuOnTop) dotsF += "<text x='"+pxf(kf)+"' y='"+(pyf(fuV)-(isLastF?8:7))+"' text-anchor='middle' font-size='"+(isLastF?10:8)+"' fill='#6dcfad' "+(isLastF?"font-weight='700'":"opacity='0.75'")+">"+fmtFuruShort(fuV)+"</text>";
          else dotsF += "<text x='"+pxf(kf)+"' y='"+(pyf(fuV)+(isLastF?18:15))+"' text-anchor='middle' font-size='"+(isLastF?10:8)+"' fill='#6dcfad' "+(isLastF?"font-weight='700'":"opacity='0.75'")+">"+fmtFuruShort(fuV)+"</text>";
        }
        if (fkV != null) {
          dotsF += "<circle cx='"+pxf(kf)+"' cy='"+pyf(fkV)+"' r='"+(isLastF?5:3)+"' fill='"+(isLastF?"#f0876a":"white")+"' stroke='#f0876a' stroke-width='2'/>";
          if (!fuOnTop) dotsF += "<text x='"+(pxf(kf)-2)+"' y='"+(pyf(fkV)-(isLastF?8:7))+"' text-anchor='middle' font-size='"+(isLastF?10:8)+"' fill='#f0876a' "+(isLastF?"font-weight='700'":"opacity='0.75'")+">"+fmtFuruShort(fkV)+"</text>";
          else dotsF += "<text x='"+(pxf(kf)-2)+"' y='"+(pyf(fkV)+(isLastF?18:15))+"' text-anchor='middle' font-size='"+(isLastF?10:8)+"' fill='#f0876a' "+(isLastF?"font-weight='700'":"opacity='0.75'")+">"+fmtFuruShort(fkV)+"</text>";
        }
      }
      var svgHf = Hf+20;
      var legendF = "<text x='"+Pf+"' y='"+(svgHf-2)+"' font-size='10' fill='#6dcfad'>■ 受入額</text><text x='"+(Pf+55)+"' y='"+(svgHf-2)+"' font-size='10' fill='#f0876a'>■ 住民税控除額</text><text x='"+Pf+"' y='"+(svgHf+9)+"' font-size='8' fill='#aaa'>※寄附があった年（H30〜R7）。住民税控除は翌年度に反映</text>";
      var spSvgF = document.getElementById("spSvg");
      spSvgF.setAttribute("viewBox","0 0 "+Wf+" "+(svgHf+10));
      spSvgF.style.height=(svgHf+10)+"px";
      spSvgF.innerHTML =
        "<path d='"+fuLine+"' fill='none' stroke='#6dcfad' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'/>" +
        "<path d='"+fkLine+"' fill='none' stroke='#f0876a' stroke-width='2' stroke-dasharray='4,3' stroke-linecap='round' stroke-linejoin='round'/>" +
        dotsF + legendF;
      var yrLabelsF = ["H30","R1","R2","R3","R4","R5","R6","R7"].slice(0, fuH.length);
      document.getElementById("spLabels").innerHTML = yrLabelsF.map(function(y,i){
        var pct = yrLabelsF.length>1 ? (14+(i/(yrLabelsF.length-1))*(300-28))/300*100 : 50;
        return "<span style='left:"+pct+"%;text-align:center;'>"+y+"</span>";
      }).join("");
    ...
    if (rankableKeys[key] && cur && DB) {
      ...
      if (isPrefView) {
        ...
        if (myRankN) {
          ...
          linesG += "<div style='display:inline-flex;align-items:center;gap:6px;background:"+colorN+"18;border:1.5px solid "+colorN+"44;border-radius:12px;padding:6px 14px;margin:0 0 6px;font-size:14px;font-weight:700;color:"+colorN+";'>"+(medalGN?medalGN+" ":"")+"🏆 全国 <span style='font-size:17px;'>"+myRankN+"位</span>/"+natListG.length+"自治体中</div>";
        }
        if (myRankP) {
          ...
          linesG += "<div style='display:inline-flex;align-items:center;gap:6px;background:"+colorP+"18;border:1.5px solid "+colorP+"44;border-radius:12px;padding:6px 14px;margin:0;font-size:14px;font-weight:700;color:"+colorP+";'>"+(medalGP?medalGP+" ":"")+"🏆 "+cur.p+"内 <span style='font-size:17px;'>"+myRankP+"位</span>/"+list.length+"自治体中</div>";
        }
