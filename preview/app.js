...
if (key === "growth" && cur && cur.pop) {
  ...
  function nkDistHtmlFromKeys(keys, title, clickableId){
    ...
    if (clickableId) html += "<p style='font-size:11px;color:#999;margin:-4px 0 8px;'>※項目タップで該当自治体を表示します</p>";
  ...
  function nkZoneListHtml(clickableId, zoneKey, regionName){
    ...
    var list = keys.map(function(k){ return {k:k, v:mmScoreOf(k)}; })
      .filter(function(o){ return o.v != null && healthState(o.v) === zoneKey; });
    list.sort(function(a,b){ return b.v - a.v; });
    if (!list.length) return "<p style='font-size:12px;color:#999;text-align:center;padding:8px 0;'>該当する自治体はありません</p>";
    var reg = mmLoad();
    var mineSet = {};
    if (reg && reg.city) mineSet[reg.city] = true;
    if (reg && reg.pref) mineSet[reg.pref] = true;
    var lbl = HEALTH_LABELS[zoneKey];
    var html = "<p style='font-size:12px;font-weight:700;color:"+lbl[1]+";margin:8px 0 2px;'>「"+lbl[0]+"」の自治体（"+list.length+"件）</p>";
    if (regionName) html += "<p style='font-size:11px;color:#999;margin:0 0 8px;'>※"+regionName+"ブロック内での順位です</p>";
    for (var i=0; i<list.length; i++){
      html += nkPrefRow(list, i, mineSet, null, true);
    }
    return html;
  }
  function nkDistHtml(){
  ...
  function nkPrefRow(list, idx, mineSet, natRank, showPref){
    var o = list[idx];
