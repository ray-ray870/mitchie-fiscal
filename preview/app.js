...
if (key === "growth" && cur && cur.pop) {
    ...
    if (key === "future" && cur) {
      ...
      if (KK && KK[curName] && KK[curName].ka3 != null) {
        ...
        if (!ka3Near) {
          var ka3Judge, ka3Note;
          if (!uHighF && !ka3High) { ka3Judge = "中央値より低め"; ka3Note = "将来負担・老朽化どちらも軽めです"; }
          else if (uHighF && ka3High) { ka3Judge = "中央値より高め"; ka3Note = "老朽化した施設の更新をこれから借金で行うと、将来負担がさらに増える可能性があります"; }
          else if (!uHighF && ka3High) { ka3Judge = "中央値より高め"; ka3Note = "①投資が行われず老朽化対策が先送りの可能性　②既存施設の活用で財政負担を抑えている可能性"; }
          else { ka3Judge = "中央値より低め"; ka3Note = "借金をしてでも施設の更新・整備を積極的に進めている可能性があります"; }
          kkFutureItems.push({icon:"🏚️", label:"老朽化率", val:ka3v+"%", judge:ka3Judge, note:ka3Note});
        }
      }
      if (KK && KK[curName] && KK[curName].ka1 != null) {
        ...
        if (!ka1Near4) {
          var ka1Judge4, ka1Note;
          if (!uHighF && ka1High4) { ka1Judge4 = "中央値より高め"; ka1Note = "借金に頼らず資産を築けています"; }
          else if (uHighF && ka1High4) { ka1Judge4 = "中央値より高め"; ka1Note = "大型の資産整備を借金でまかなってきた可能性があります"; }
          else if (uHighF && !ka1High4) { ka1Judge4 = "中央値より低め"; ka1Note = "資産形成に見合わない借金を抱えている可能性があります"; }
          else { ka1Judge4 = "中央値より低め"; ka1Note = "大きな整備は行っていませんが、身の丈に合った運営とも言えます"; }
          kkFutureItems.push({icon:"💰", label:"住民一人当たり資産額", val:ka1v4+"万円", judge:ka1Judge4, note:ka1Note});
        }
      }
      if (KK && KK[curName] && KK[curName].ka6 != null) {
        ...
        if (!ka6Near4) {
          var ka6Judge4, ka6Note;
          if (!uHighF && !ka6High4) { ka6Judge4 = "中央値より低め"; ka6Note = "コンパクトな運営と言えます"; }
          else if (uHighF && ka6High4) { ka6Judge4 = "中央値より高め"; ka6Note = "サービス水準を維持するための借金が将来負担として残っている可能性があります"; }
          else if (!uHighF && ka6High4) { ka6Judge4 = "中央値より高め"; ka6Note = "手厚いサービスを借金に頼らず提供できています"; }
          else { ka6Judge4 = "中央値より低め"; ka6Note = "過去の投資の返済が今の身軽さと引き換えになっている可能性があります"; }
          kkFutureItems.push({icon:"🧑‍💼", label:"住民一人当たり行政コスト", val:ka6v4+"万円", judge:ka6Judge4, note:ka6Note});
        }
      }
      topSummaryHtml += kkCrossBoxMulti("🏛️ 総務省公式の組み合わせ分析",
        "将来への借金", cur.u+"%", uHighF?"重め":"軽め", kkFutureItems);
    }
...
/* ===== 公会計機能 ===== */
  ...
  // 財政側の指標1つに対して、公会計側の複数指標をまとめて箇条書きで表示する（僅差の項目は呼び出し側で除外して渡す）
  function kkCrossBoxMulti(heading, zaiLabel, zaiVal, zaiJudge, items) {
    if (items.length === 0) return "";
    var rows = items.map(function(it){
      return "<div style='background:white;border-radius:10px;padding:12px 14px;margin-top:8px;'>" +
        "<div style='display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px;'>" +
          "<span style='font-size:13px;color:#c0623a;font-weight:700;'>" + it.icon + " [公会計] " + it.label + "</span>" +
          "<span style='font-size:17px;font-weight:700;color:#2a2a3a;'>" + it.val + "</span>" +
        "</div>" +
        "<div style='font-size:13px;color:#5a5a7a;line-height:1.6;'>・" + it.judge + "→" + it.note + "</div>" +
      "</div>";
    }).join("");
    return "<div style='background:#e8f7f0;border:1px solid #6dcfad55;border-radius:14px;padding:16px;margin-top:10px;'>" +
      "<div style='font-size:15px;font-weight:700;color:#3a9970;margin-bottom:12px;'>" + heading + "</div>" +
      "<div style='background:white;border-radius:12px;padding:12px 14px;text-align:center;margin-bottom:4px;'>" +
        "<div style='font-size:12px;color:#3a9970;font-weight:700;margin-bottom:4px;'>[財政] " + zaiLabel + "</div>" +
        "<div style='font-size:22px;font-weight:700;color:#2a2a3a;'>" + zaiVal + "</div>" +
        "<div style='font-size:12px;color:#888;'>" + zaiJudge + "</div>" +
      "</div>" +
      rows +
    "</div>";
  }
