      ...
      if (KK && KK[curName] && KK[curName].ka7 != null) {
        ...
        if (!dHigh && !ka7High) {
          ...
          var ka7ReasonA = ka7TrendA === "declining"
            ? "着実に返済が進んでいる長期返済中、という可能性も考えられます。"
          ...
          linkMsgD = ka7TrendB === "declining"
            ? "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は重めですが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体はそれほど大きくありません。短期集中で返済を終えつつある可能性も考えられます。"
...
/* ===== 公会計機能 ===== */
  ...
  function kkPerCapitaTrendNote(entry, code, curVal, g) {
    if (!entry) return "";
    var oldVal = entry[code + "_r4"];
    if (oldVal == null || curVal == null) return "";
    var pctChange = oldVal !== 0 ? (curVal - oldVal) / Math.abs(oldVal) : 0;
    if (pctChange >= 0.05) return ""; // 5%以上伸びていれば特に触れない
    var popGrowing = g != null && g > 0.3;
    if (popGrowing) {
      return "人口が増えている一方でこの金額はあまり伸びていません。人口増加に整備・サービス拡充が追いついていない可能性があります。";
    }
    return "この金額はここ数年あまり変わっていません。老朽化による目減り分と新規投資がほぼ相殺されている可能性があります。";
  }
