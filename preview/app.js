...
/* ===== 公会計機能 ===== */
  ...
  function kkReciprocalCross(code, entryV, isPref, entry) {
    ...
    if (code === "ka4" && cur.sfs && cur.sfs > 0) {
      ...
      return kkCrossBox("🔗 財政と比べてみると", "財政調整基金残高", ratioRec.toFixed(1)+"%", judgeRec, "純資産比率", entryV+"%", ka4HighRec?"中央値より高め":"中央値より低め", curName + "の財政調整基金（すぐ使える貯金）は標準財政規模の" + ratioRec.toFixed(1) + "%です。" + bodyRec, true, rTrendPhrase, ka4TrendPhrase) + KK_CROSSCHECK_CAVEAT;
    }
    if (code === "ka7" && cur.d != null) {
      ...
      return kkCrossBox("🔗 財政と比べてみると", "実質公債費比率", cur.d+"%", judgeD, "住民一人当たり負債額", entryV+"万円", ka7HighRec?"中央値より高め":"中央値より低め", curName + "の実質公債費比率（毎年の返済負担）は" + cur.d + "%です。" + bodyD, true, dTrendPhrase, ka7TrendPhrase) + KK_CROSSCHECK_CAVEAT;
    }
  ...
  function kkOpenDetail(code, skipPush){
    ...
    var ka8NoteHtml = (code === "ka8") ? "<div style='background:#a08be814;border:1px solid #a08be840;border-radius:12px;padding:12px 14px;margin-top:16px;font-size:13px;color:#5a4a80;line-height:1.7;'>この指標は、総務省の「統一的な基準による財務書類（資金収支計算書）」に基づく数値です。国の財政で使われる「プライマリーバランス」と考え方は似ていますが、対象や計算方法が異なるため、同じものではありません。このアプリでは、総務省の財務書類に基づく「業務・投資活動収支」という名称を使用しています。</div>" : "";
