  ...
  function openD(key, skipPush) {
    ...
    if (key === "furusato") {
      ...
      function buildMiniChart(arr, lineColor, textColor, labelBg, gradId){
        ...
        for (var j=0;j<n;j++){
          var isLast = j===n-1;
          dots += "<circle cx='"+pts[j][0].toFixed(1)+"' cy='"+pts[j][1].toFixed(1)+"' r='"+(isLast?4:2.5)+"' fill='"+(isLast?lineColor:"white")+"' stroke='"+lineColor+"' stroke-width='2'/>";
          if (!isLast) {
            dots += "<text x='"+pts[j][0].toFixed(1)+"' y='"+(pts[j][1]-6).toFixed(1)+"' text-anchor='middle' font-size='9.5' font-weight='600' fill='"+textColor+"'>"+fmtFuruShort(arr[j])+"</text>";
          }
        }
        ...
        var svgInner = "<defs><linearGradient id='"+gradId+"' x1='0' y1='0' x2='0' y2='1'><stop offset='0%' stop-color='"+lineColor+"' stop-opacity='0.2'/><stop offset='100%' stop-color='"+lineColor+"' stop-opacity='0'/></linearGradient></defs>" +
          ...
          dots +
          "<rect x='"+boxX.toFixed(1)+"' y='2' width='"+boxW+"' height='18' rx='9' fill='white'/>" +
        ...
        return "<svg viewBox='0 0 "+Wm+" "+Hm+"' style='width:100%;height:"+Hm+"px;'>"+svgInner+"</svg>" +
