    ...
    if (key === "reserve" && cur && cur.sfs && cur.sfs > 0) {
      ...
      if (KK && KK[curName] && KK[curName].ka4 != null) {
        ...
        topSummaryHtml += kkCrossBox("🔗 公会計と合わせて見ると",
          ...
          false, (function(){ function getRRatioF(i){ var rv=i===0?cur.r:cur["r_r"+i]; var sv=i===0?cur.sfs:cur["sfs_r"+i]; return (rv!=null&&sv)?rv/sv*100:null; } return withTrendMeaning("r", trendSincePhrase(getRRatioF, 5, 6)); })(),
          (function(){ function getKa4ValF(i){ return i===0?KK[curName].ka4:KK[curName]["ka4_r"+i]; } return withTrendMeaning("ka4", trendSincePhrase(getKa4ValF, 4, 5)); })());
...
if (key === "growth" && cur && cur.pop) {
    ...
    if (key === "debt" && cur) {
      ...
      if (KK && KK[curName] && KK[curName].ka7 != null) {
        ...
        if (ka7Near) {
          ka7Judge = "中央値とほぼ同水準";
          linkMsgD = "<strong style='color:#c0623a;'>[公会計]</strong>住民一人当たり負債額は中央値とほぼ同水準です。<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は" + (dHigh?"重め":"軽め") + "ですが、この" + (dHigh?"重さ":"軽さ") + "は負債額以外の要因によるものと考えられます。";
          ...
          linkMsgD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は軽めに見えますが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体は大きめです。" + (ka7TrendA === "declining" ? "着実に返済が進んでいる長期返済中、という可能性も考えられます。" : "新しい借入も続いており、返済はこれから本格化する可能性も考えられます。");
        } else {
          ka7Judge = "中央値より低め";
          var ka7TrendB = kkMetricTrend(KK[curName], "ka7", ka7v);
          linkMsgD = "<strong style='color:#3a9970;'>[財政]</strong>毎年の返済負担は重めですが、<strong style='color:#c0623a;'>[公会計]</strong>借金の残高自体はそれほど大きくありません。" + (ka7TrendB === "declining" ? "短期集中で返済を終えつつある可能性も考えられます。" : "返済期間を短く設定している可能性も考えられます。");
        }
        function getDValF(i){ return i===0 ? cur.d : cur["d_r"+i]; }
        var dTrendPhraseF = withTrendMeaning("d", trendSincePhrase(getDValF, 5, 6));
        function getKa7ValF(i){ return i===0 ? KK[curName].ka7 : KK[curName]["ka7_r"+i]; }
        var ka7TrendPhraseF = withTrendMeaning("ka7", trendSincePhrase(getKa7ValF, 4, 5));
        topSummaryHtml += kkCrossBox("🔗 公会計と合わせて見ると",
    ...
    if (key === "future" && cur) {
      ...
      ["ka3","ka1","ka6"].forEach(function(code){
        if (futureEntry && futureEntry[code] != null) {
          var boxHtml = buildFutureComboBox(code, futureEntry[code], isPrefView, cur, curName, futureEntry, false);
          if (boxHtml) { topSummaryHtml += boxHtml; kkCrossBoxShownF = true; }
        }
      });
...
/* ===== 公会計機能 ===== */
  ...
  var FUTURE_COMBO_META = {
    ka3: {label:"施設の老朽化率", fullLabel:"有形固定資産減価償却率（老朽化率）", unit:"%", nearNoun:"老朽化率"},
  ...
  function buildFutureComboBody(code, kkVal, med, uHigh, entry) {
    ...
    if (near) {
      judge = "中央値とほぼ同水準";
      body = "<strong style='color:#c0623a;'>[公会計]</strong>" + m.label + "は中央値とほぼ同水準です。<strong style='color:#3a9970;'>[財政]</strong>将来への借金は" + (uHigh?"重め":"軽め") + "ですが、この" + (uHigh?"重さ":"軽さ") + "は" + m.nearNoun + "以外の要因によるものと考えられます。";
      return {judge:judge, body:body};
    }
    if (code === "ka3") {
      if (!uHigh && !high) { judge="中央値より低め"; body="<strong style='color:#3a9970;'>[財政]</strong>将来への借金・<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化、どちらの面から見ても軽い状態です。"; }
      else if (uHigh && high) { judge="中央値より高め"; body="<strong style='color:#3a9970;'>[財政]</strong>将来への借金・<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化、どちらの面から見ても重い状態です。老朽化した施設の更新をこれから借金で行うと、将来負担がさらに増える可能性があります。"; }
      else if (!uHigh && high) {
        judge = "中央値より高め";
        var ka1Trend = kkMetricTrend(entry, "ka1", entry ? entry.ka1 : null);
        var reason = ka1Trend === "growing"
          ? "②既存施設を活用しながら、新しい整備も進めている可能性も考えられます。"
          : "①必要な投資が行われず、老朽化対策が先送りにされている可能性も考えられます。";
        body = "<strong style='color:#3a9970;'>[財政]</strong>将来への借金は軽めですが、<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化は進んでいます。" + reason;
      }
      else { judge="中央値より低め"; body="<strong style='color:#3a9970;'>[財政]</strong>将来への借金は重めですが、<strong style='color:#c0623a;'>[公会計]</strong>施設の老朽化は進んでいません。借金をしてでも施設の更新・整備を積極的に進めている可能性があります。"; }
    } else if (code === "ka1") {
      if (!uHigh && high) { judge="中央値より高め"; body="<strong style='color:#c0623a;'>[公会計]</strong>資産規模は大きめですが、<strong style='color:#3a9970;'>[財政]</strong>将来への借金は軽めです。借金に頼らず資産を築けています。"; }
      else if (uHigh && high) { judge="中央値より高め"; body="<strong style='color:#c0623a;'>[公会計]</strong>資産規模・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも大きめです。大型の資産整備を借金でまかなってきた可能性があります。"; }
      else if (uHigh && !high) { judge="中央値より低め"; body="<strong style='color:#c0623a;'>[公会計]</strong>資産規模は控えめですが、<strong style='color:#3a9970;'>[財政]</strong>将来への借金は重めです。資産形成に見合わない借金を抱えている可能性があります。"; }
      else { judge="中央値より低め"; body="<strong style='color:#c0623a;'>[公会計]</strong>資産規模・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも小さめです。資産の規模と将来世代への負担のバランスは取れている状態です。"; }
    } else {
      if (!uHigh && !high) { judge="中央値より低め"; body="<strong style='color:#c0623a;'>[公会計]</strong>行政コスト・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも軽めです。コンパクトな運営と言えます。"; }
      else if (uHigh && high) { judge="中央値より高め"; body="<strong style='color:#c0623a;'>[公会計]</strong>行政コスト・<strong style='color:#3a9970;'>[財政]</strong>将来への借金、どちらも重めです。サービス水準を維持するための借金が将来負担として残っている可能性があります。"; }
      else if (!uHigh && high) { judge="中央値より高め"; body="<strong style='color:#c0623a;'>[公会計]</strong>行政コストは高めですが、<strong style='color:#3a9970;'>[財政]</strong>将来への借金は軽めです。手厚いサービスを借金に頼らず提供できています。"; }
      else { judge="中央値より低め"; body="<strong style='color:#c0623a;'>[公会計]</strong>行政コストは抑えめですが、<strong style='color:#3a9970;'>[財政]</strong>将来への借金は重めです。過去の投資の返済が今の身軽さと引き換えになっている可能性があります。"; }
    }
    return {judge:judge, body:body};
  }
  function buildFutureComboBox(code, kkVal, isPrefView, curObj, curNameStr, entry, reverseOrder) {
    if (kkVal == null || curObj.u == null) return "";
    var m = FUTURE_COMBO_META[code];
    var med = isPrefView ? KK_MEDIANS[code].pref : KK_MEDIANS[code].muni;
    var uHigh = curObj.u > 100;
    var r = buildFutureComboBody(code, kkVal, med, uHigh, entry);
    function getUVal(i){ return i===0 ? curObj.u : curObj["u_r"+i]; }
    var uTrendPhrase = withTrendMeaning("u", trendSincePhrase(getUVal, 5, 6));
    function getKkVal(i){ return i===0 ? entry[code] : entry[code+"_r"+i]; }
    var kkTrendPhrase = entry ? withTrendMeaning(code, trendSincePhrase(getKkVal, 4, 5)) : null;
    return kkCrossBox("🏛️ 総務省公式の組み合わせ分析",
      "将来負担比率", curObj.u+"%", uHigh?"重め":"軽め",
      m.label, kkVal+m.unit, r.judge,
      curNameStr + "は公会計でみた" + m.fullLabel + "が" + kkVal + m.unit + "です。" + r.body,
