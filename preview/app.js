...
/* ===== 公会計機能 ===== */
  ...
  function buildFutureComboBox(code, kkVal, isPrefView, curObj, curNameStr, entry, reverseOrder, boxNumber) {
    ...
    return kkCrossBox("🔗 組み合わせ分析" + numMark,
  ...
  function kkReciprocalCross(code, entryV, isPref, entry) {
    ...
    if (code === "ka4" && cur.sfs && cur.sfs > 0) {
      ...
      return kkCrossBox("🔗 組み合わせ分析", "財政調整基金残高", ratioRec.toFixed(1)+"%", judgeRec, "純資産比率", entryV+"%", ka4HighRec?"中央値より高め":"中央値より低め", curName + "の財政調整基金（すぐ使える貯金）は標準財政規模の" + ratioRec.toFixed(1) + "%です。" + bodyRec, true, rTrendPhrase, ka4TrendPhrase) + KK_CROSSCHECK_CAVEAT;
    }
    if (code === "ka7" && cur.d != null) {
      ...
      return kkCrossBox("🔗 組み合わせ分析", "実質公債費比率", cur.d+"%", judgeD, "住民一人当たり負債額", entryV+"万円", ka7HighRec?"中央値より高め":"中央値より低め", curName + "の実質公債費比率（毎年の返済負担）は" + cur.d + "%です。" + bodyD, true, dTrendPhrase, ka7TrendPhrase) + KK_CROSSCHECK_CAVEAT;
    }
