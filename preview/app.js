(function() {
  ...
  var DATA_VER = "202609141830"; // データ更新のたびにここを書き換えてキャッシュを回避する
  Promise.all(dataFiles.map(function(f){ return fetch(f+"?v="+DATA_VER).then(function(r){ return r.json(); }); }).concat([fetch("kokaikei.json?v="+DATA_VER).then(function(r){ return r.json(); }).catch(function(){ return {}; })]))
    ...
    .catch(function(e){
      ...
      window.mitchieLoadError = String(e && e.message ? e.message : e);
    });
  ...
  function diagnose() {
    ...
    if (!q.trim()) return;
    if (!DB) { alert("データを読み込み中です。少し待ってからもう一度お試しください。" + (window.mitchieLoadError ? ("\n\n[エラー詳細] " + window.mitchieLoadError) : "")); return; }
...
if (key === "growth" && cur && cur.pop) {
    ...
    var trendYrLabelsCovid = covidReasonsPeak[key] ? (hasHistory ? yrs.map(function(y){ return y.replace("（最新）",""); }) : hasGrowthHistory ? growthYrLabels.slice(0, vals.length) : null) : null;
    if (trendYrLabelsCovid && vals.length >= 2) {
      ...
      if (validIdxCovid.length >= 2) {
        ...
        if (peakICovid !== lastValidCovid && covidYrsSet[trendYrLabelsCovid[peakICovid]]) {
          topSummaryHtml += "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+trendYrLabelsCovid[peakICovid]+"年度が最も高い年です。新型コロナウイルス対応の時期と重なり、"+covidReasonsPeak[key]+"</div>";
        } else if (troughICovid !== lastValidCovid && covidYrsSet[trendYrLabelsCovid[troughICovid]]) {
          topSummaryHtml += "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+trendYrLabelsCovid[troughICovid]+"年度が最も低い年です。新型コロナウイルス対応の時期と重なり、"+covidReasonsTrough[key]+"</div>";
        }
      }
    }
  ...
  function kkOpenDetail(code, skipPush){
    ...
    if (kkVals.length >= 2 && kkVals.every(function(v){ return v != null; })) {
      ...
      if (kkCovidReasonsPeak[code] && kkPeakI !== kkLastI && kkCovidYrsSet[kkYrLabels[kkPeakI]]) {
        kkCovidNote = "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+kkYrLabels[kkPeakI]+"年度が最も高い年です。新型コロナウイルス対応の時期と重なり、"+kkCovidReasonsPeak[code]+"</div>";
      } else if (kkCovidReasonsTrough[code] && kkTroughI !== kkLastI && kkCovidYrsSet[kkYrLabels[kkTroughI]]) {
        kkCovidNote = "<div style='font-size:12px;color:#9090a8;margin-top:6px;'>📅 "+kkYrLabels[kkTroughI]+"年度が最も低い年です。新型コロナウイルス対応の時期と重なり、"+kkCovidReasonsTrough[code]+"</div>";
      }
