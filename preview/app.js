...
if (key === "growth" && cur && cur.pop) {
  ...
  function mmMakeSuggest(inputId, boxId, isCity){
    ...
    function update(){
      ...
      // 読み仮名（ひらがな）の前方一致も候補に加える
      if (KANA_INDEX) {
        Object.keys(KANA_INDEX).forEach(function(kana){
          if (kana.indexOf(q) === 0) {
            KANA_INDEX[kana].forEach(function(ck){
              if (!seen[ck] && DB[ck] && (isCity ? !mmIsPrefKey(ck) : mmIsPrefKey(ck))) { starts.push(ck); seen[ck]=true; }
            });
          }
        });
      }
