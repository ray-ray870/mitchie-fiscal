...
if (key === "growth" && cur && cur.pop) {
    ...
    if (key !== "budget" && (hasHistory || hasGrowthHistory)) {
      ...
      if (missingYearLabels.length) {
        missingNoteHtml = "<div style='font-size:10px;color:#c98a3a;padding-top:22px;line-height:1.5;'>⚠️ "+missingYearLabels.join("・")+"年度は総務省の公表データが無いため表示していません（前後の実データを直線ではつないでいません）</div>";
      }
    }
    // 過去の実績データが1件も無い場合：架空の推計線は描かず、その旨だけをはっきり伝える
    if (key !== "budget" && noRealHistory) {
      missingNoteHtml = "<div style='font-size:12px;color:#8a8a9a;margin-top:4px;line-height:1.6;'>📭 過去の推移データは総務省の公表資料に無いため、表示できません（現在値のみ上に表示しています）</div>";
    }
    document.getElementById("spLabels").innerHTML = yrsLabelHtml + missingNoteHtml;
    if (key === "budget") {
      // 歳出・歳入 2本線グラフ
      ...
      var budgetMissingNote = noBudgetHistory ? "<div style='font-size:12px;color:#8a8a9a;padding-top:22px;line-height:1.6;'>📭 過去の歳出入の推移データは総務省の公表資料に無いため、表示できません（現在値のみ上に表示しています）</div>" : "";
