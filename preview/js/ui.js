  ...
  function openD(key, skipPush) {
    ...
    if (hasHistory) {
      ...
      // 推計（従来ロジック）：ラベルはR1〜R4(推計)+R5(最新・実値)の5個なので、
      // 推計値も4個だけ作る（以前は5個作った後に実値を足して6個になり、ラベル数とズレていた）
      yrs = ["R1","R2","R3","R4","R5（最新）"];
      var v = val * (0.88 + (seed%20)/100);
      for (var i=0; i<4; i++) { v = v + Math.sin(i*2.1+seed*0.1)*Math.abs(val)*0.08; vals.push(parseFloat(v.toFixed(1))); }
      vals.push(val);
    }
...
if (key === "growth" && cur && cur.pop) {
    ...
    } else if (hasHistory || hasGrowthHistory) {
      noteText = "";
    } else {
      noteText = key==="growth" ? "※元〜4年は参考値（推計）※町村はデータなし" : "※元〜4年は参考値（推計）";
    }
