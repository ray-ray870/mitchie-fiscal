...
if (key === "growth" && cur && cur.pop) {
    ...
    if (key !== "budget" && (hasHistory || hasGrowthHistory)) {
      ...
      if (missingYearLabels.length) {
        missingNoteHtml = "<div style='font-size:10px;color:#c98a3a;margin-top:8px;line-height:1.5;'>⚠️ "+missingYearLabels.join("・")+"年度は総務省の公表データが無いため表示していません（前後の実データを直線ではつないでいません）</div>";
      }
    }
    ...
    if (key === "budget") {
      ...
      var budgetMissingNote = noBudgetHistory ? "<div style='font-size:12px;color:#8a8a9a;margin-top:4px;line-height:1.6;'>📭 過去の歳出入の推移データは総務省の公表資料に無いため、表示できません（現在値のみ上に表示しています）</div>" : "";
