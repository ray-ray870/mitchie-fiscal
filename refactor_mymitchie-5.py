...
SHARERANK_OLD = "\"<div class='sc-state-label' style='color:\"+pr.c+\";'>\"+stateLabel+\"</div>\" +\n        \"</div>\" +\n"
SHARERANK_NEW = "\"<div class='sc-state-label' style='color:\"+pr.c+\";'>\"+stateLabel+\"</div>\" +\n          shareRankBoxHtml(cityName, d) +\n        \"</div>\" +\n"
SHARETOPRIGHT_CSS_OLD = ".share-card .sc-top-right{flex:1;padding-left:34px;display:flex;flex-direction:column;justify-content:center;}"
SHARETOPRIGHT_CSS_NEW = ".share-card .sc-top-right{flex:1;padding-left:34px;display:flex;flex-direction:column;justify-content:space-between;}"
...
if CNAME_OLD not in js:
    ...
    sys.exit(1)
if SHARERANK_OLD not in js:
    print("::error::app.js 内に共有カードのstate-label位置が見つかりません。")
    sys.exit(1)
if SHARETOPRIGHT_CSS_OLD not in css:
    print("::error::style.css 内にsc-top-rightが見つかりません。")
    sys.exit(1)

# --- 差し込み ---
new_html = html.replace(HTML_ANCHOR, mm_html + "\n" + HTML_ANCHOR, 1)
new_css = css.replace(CORNERWRAP_CSS_OLD, CORNERWRAP_CSS_NEW, 1)
new_css = new_css.replace(SHARETOPRIGHT_CSS_OLD, SHARETOPRIGHT_CSS_NEW, 1) + mm_css
...
js2 = js2.replace(SHARERANK_OLD, SHARERANK_NEW, 1)
...
checks = [
    ...
    ("JSにshareRankBoxHtmlが入っている", "shareRankBoxHtml" in new_js),
