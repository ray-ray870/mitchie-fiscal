...
# 同名自治体の区別（アプリ側のキーに合わせる）
NAME_FIX = {
    ("東京都", "府中市"): "府中市（東京）",
    ("広島県", "府中市"): "府中市（広島）",
}

SFS_ROW, SFS_LABEL_COL, SFS_VALUE_COL = 54, 84, 97
...
def read_card(data, pref):
    """決算カードのExcelから {自治体名: 標準財政規模(億円)} を作る。"""
    ...
    for sh in wb.sheetnames:
        if sh == "目次":
            continue
        ws = wb[sh]
        if ws.cell(SFS_ROW, SFS_LABEL_COL).value != "標準財政規模":
            skipped += 1
            continue
        v = ws.cell(SFS_ROW, SFS_VALUE_COL).value
        if not isinstance(v, (int, float)):
            skipped += 1
            continue
        name = re.sub(r"^[0-9]+", "", sh).strip()
        name = NAME_FIX.get((pref, name), name)
        out[name] = round(v / 100000.0, 1)  # 千円 → 億円
    wb.close()
    return out, skipped
    ...
    for i, (pref, url) in enumerate(build_urls(base), 1):
        ...
        try:
            ...
            fail("%s の取得に失敗しました: %s\n"
                 ...
                 "直してください。" % (pref, e))
        got, skipped = read_card(data, pref)
        dup = set(got) & set(all_sfs)
        if dup:
            print("    ※ 名前の重複: %s" % ", ".join(sorted(dup)))
        all_sfs.update(got)
