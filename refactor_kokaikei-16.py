...
FISCALPOWER_DUP_OLD = "🟠 0.25未満 → 税収基盤が特に弱い\\n\\nなお1.0以上は不交付団体（地方交付税が交付されない団体）です。\\n\\nこの数値が低いことは、"
FISCALPOWER_DUP_NEW = "🟠 0.25未満 → 税収基盤が特に弱い\\n\\nこの数値が低いことは、"

OPEND_START_OLD = "  function openD(key, skipPush) {\n    if (!cur) return;\n    var m = META[key];\n    if (!m) return;\n"
OPEND_START_NEW = (
...
SH_QUERY_OLD = 'document.querySelector(".sh")'
SH_QUERY_NEW = 'document.querySelector("#ovEl .sh")'
...
DESC_FINAL_NEW = 'document.getElementById("shTop").innerHTML = rankHtml + topSummaryHtml; document.getElementById("shDesc").innerHTML = ((rankHtml || topSummaryHtml) ? "<div style=\'border-top:1px dashed #d8d5e8;margin:22px 0;\'></div><div style=\'font-size:16px;font-weight:700;color:#3a6ee8;margin-bottom:8px;\'>"+m.label+"とは？</div>" : "") + descHtml;'
...
CHAIN_MODE = MODE == "preview" and os.path.exists(os.path.join("preview", "app.js"))
SOURCE_DIR = "preview" if CHAIN_MODE else "."
if CHAIN_MODE:
    print("::notice::preview/ に既存の出力（他機能のpreview）を検出したため、そちらを土台にして重ねます。")

for f in FILES:
    if not os.path.exists(os.path.join(SOURCE_DIR, f)):
        print(f"::error::{os.path.join(SOURCE_DIR, f)} が見つかりません。")
        sys.exit(1)
...
js = open(os.path.join(SOURCE_DIR, "app.js"), encoding="utf-8").read()
html = open(os.path.join(SOURCE_DIR, "index.html"), encoding="utf-8").read()
css = open(os.path.join(SOURCE_DIR, "style.css"), encoding="utf-8").read()
...
if js.count(FISCALSTATUS_OLD) != 1:
    ...
    sys.exit(1)
if js.count(FISCALPOWER_DUP_OLD) != 1:
    print("::error::app.js 内に財政力指数の重複説明箇所が一意に見つかりません。")
    sys.exit(1)
if js.count(OPEND_START_OLD) != 1:
    print("::error::app.js 内にopenD関数の開始位置が一意に見つかりません。")
    sys.exit(1)
if js.count(SH_QUERY_OLD) < 1:
    print("::error::app.js 内に.sh要素の取得箇所が見つかりません。")
    sys.exit(1)
...
if POPSTATE_OLD not in js:
    ...
    sys.exit(1)
if "})();" not in js:
    print("::error::app.js 内にIIFEの終端位置が見つかりません。")
    sys.exit(1)
...
new_js = new_js.replace(FISCALPOWER_DUP_OLD, FISCALPOWER_DUP_NEW, 1)
new_js = new_js.replace(OPEND_START_OLD, OPEND_START_NEW, 1)
new_js = new_js.replace(SH_QUERY_OLD, SH_QUERY_NEW)
_last_iife = new_js.rfind("})();")
if _last_iife == -1:
    print("::error::app.js 内にIIFEの終端位置が見つかりません（差し込み直前の再チェック）。")
    sys.exit(1)
new_js = new_js[:_last_iife] + kk_js.strip() + "\n\n" + new_js[_last_iife:]
new_css = css + kk_css
new_html = html.replace(HTML_OLD, HTML_NEW, 1)

# キャッシュ対策：実行のたびにバージョン番号を更新し、ブラウザの古いキャッシュを確実に回避する
import re as _re
import time as _time
_new_ver = _time.strftime("%Y%m%d%H%M")
new_html = _re.sub(r'(\.(?:js|css)\?v=)\d+', r'\g<1>' + _new_ver, new_html)
...
checks = [
    ...
    ("index.htmlのバージョン番号が更新されている", "v=" + _new_ver in new_html),
]
ok = True
...
if MODE == "preview":
    outdir = "preview"
    if CHAIN_MODE:
        # 既存のpreview（他機能分）を消さずに重ねる。root側にしかないアセットだけ追加コピー。
        for name in ROOT_ASSETS:
            dest = os.path.join(outdir, name)
            if not os.path.exists(dest) and os.path.exists(name):
